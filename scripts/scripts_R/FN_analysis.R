...
## Load data
load(paste0(results.path, "/model_genus_MFDO1.RData"))
...
FN.plot <- FN_dist %>%
  ...
  facet_wrap(hab_class ~ ., scales = "free_y")
...
FN.plot <- FN_dist %>%
  ...
  facet_wrap(hab_class ~ ., scales = "free_y", nrow = 2) +
  labs(x = "MFDO1 habitat", y = "False Negative")

ggsave(FN.plot, filename = paste0(results.path, "/FN.png"), width = 15, height = 8, dpi = "retina")
ggsave(FN.plot, filename = paste0(results.path, "/FN.svg"), width = 15, height = 8, dpi = "retina")
